...
# ms.tgt_pltfrm: 
ms.custom: 70381
ms.assetid: 1bf97a80-7c13-4ad4-9f30-64bd39e82115
# ms.region: 
# ms.industry: 
ms.author: maertenm
...
1.  Create an ISV solution. ![Creating an ISV solution]
2.  Add the certificate's public key (.cer file) to your project as a resource.
    1.  Add a new item. ![Adding a new item]
    2.  Click **Labels And Resources**, and then click **Resource**. ![Clicking Resource]
    ...
    4.  Add the certificate as a resource. ![Adding the certificate as a resource]

3.  Create a license code. ![Creating a license code]
4.  Map the certificate to the license code. ![Mapping the certificate to the license code]
5.  Create one or more configuration keys. ![Creating a configuration key]
6.  Associate the license code with the configuration keys. ![Associating the license code with the configuration keys]
7.  Associate a configuration key to an element in your solution. For example, create a new form. ![Creating a new form]
8.  Add a button to the form. ![Adding a button to the new form](media/11-1024x553.png) The button will be visible, because it isn't controlled by a configuration key at first. [![New button is visible when it's first added](media/12.png)](media/12.png)
9.  Associate a configuration key with the button. ![Associating a configuration key with the button] The button will no longer be visible, because the configuration key must be available and enabled. ![Button is no longer visible]
