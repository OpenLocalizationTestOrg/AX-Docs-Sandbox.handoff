...
title: Segmented Entry controls – Migration guidance | Microsoft Docs
...
# ms.tgt_pltfrm: 
ms.custom: 25651
ms.assetid: 7fcae563-43c2-4728-9d0f-88394bf45b44
# ms.region: 
# ms.industry: 
ms.author: annbe
...
# Segmented Entry controls – Migration guidance
...
![SegmentMigrate04] ![SegmentMigrate05] **Note:** A controller class is required for the control to work. Therefore, a run-time error will be thrown if the **Controller Class** property isn't set.
...
This is the **Posting Type** property on the control. The control that the **PostingType** property must be set on can be determined from the mapping details that are derived by looking at the **parmControl()** call. ![SegmentMigrate06] These properties can also be set in code, through corresponding **parm** methods on the control instance. Here's an example.
...
Override **resolveReference()** in the data source field for the ledger dimension. ![SegmentMigrate07]
